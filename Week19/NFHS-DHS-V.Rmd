...
```{r}
library(ggplot2)
g <- ggplot(df, aes(fill=as.factor(km2$cluster)))+theme(legend.title=element_blank())

# use gridExtra library to make 2x2 plots here
library(gridExtra)
 
p1 <- g + geom_bar(aes(HV206), position = "dodge")+labs(x="HV206: electricity")
p2 <- g + geom_bar(aes(HV209), position = "dodge")+labs(x="HV209: refrigerator")
p3 <- g + geom_bar(aes(HV243A), position = "dodge")+labs(x="HV243A: cell phone")
p4 <- g + geom_bar(aes(HV025), position = "dodge")+labs(x="HV025: rural(0) or urban(1)")
...
km5 <- kmeans(df_full[,1:45],5,nstart=3) # larger nstart throws a memory error
